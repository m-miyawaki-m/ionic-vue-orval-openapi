...
6シート: `01_表紙`（メタ＋使い方＋凡例OK/NG/-/保留＋改訂履歴＋**利用データ**: テストアカウント/環境/MSW固定フィクスチャ/境界値データの所在）/ カテゴリ別チェックリスト
`02_画面表示`・`03_ボタン操作`・`04_API連携`・`05_権限・認証`（9列: No./区分/テスト項目/確認手順/
期待結果/結果(OK/NG)/NG時の内容・備考/エビデンスNo./実施日、結果列は OK/NG/-/保留 ドロップダウン）/
`06_エビデンス`（番号付きブロック＋スクショ貼付枠）。
※現状は**空雛形**。cases.json/テスト結果からの自動充填は次段の予定。
